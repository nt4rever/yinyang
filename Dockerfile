...
COPY --link --chown=${WWWUSER}:${WWWUSER} deployment/supervisord.roadrunner.conf /etc/supervisor/conf.d/
...
COPY --link --chown=${WWWUSER}:${WWWUSER} deployment/.rr.prod.yaml ./.rr.yaml
